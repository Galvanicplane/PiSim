// Shared UDP message definition for PiSim / Raspberry Pi interoperability.
#pragma once

#include "CoreMinimal.h"
#include "UDPMessage.generated.h"

USTRUCT(BlueprintType)
struct PISIM_API FUDPMessage
{
    GENERATED_BODY()

    UPROPERTY(BlueprintReadWrite, Category = "UDP")
    FString MessageType = TEXT("Telemetry");
    ...
    static bool FromBinary(const TArray<uint8>& Bytes, FUDPMessage& OutMessage)
    {
        ...
        if (!ReadFloat(OutMessage.Yaw))
        {
            return false;
        }
        if (!ReadFloat(OutMessage.Velocity))
        {
            return false;
        }
